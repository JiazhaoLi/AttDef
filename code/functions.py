...
def evaluate(model, tokenizer, eval_text_list, eval_label_list, batch_size, criterion, device):
    ...
    with torch.no_grad():
        for i in range(NUM_EVAL_ITER):
            ...
            pre.extend(list(rounded_preds.detach().cpu().numpy()))
            label_list.extend(list(labels.detach().cpu().numpy()))
            
            
            log_list.extend(list(outputs.logits.detach().cpu().numpy()))
            acc_num, acc = binary_accuracy(outputs.logits, labels)
            epoch_loss += loss.item() * len(batch_sentences)
            epoch_acc_num += acc_num

    
    return epoch_loss / total_eval_len, epoch_acc_num / total_eval_len, pre, label_list,log_list
